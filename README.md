...
npm run env:load
# На Windows. Для других ОС см. примечания ниже
...
npm run forge:install    # первый раз; site — в package.json
npm run forge:upgrade    # после изменения manifest.yml
```

Для другого Atlassian-сайта измени `--site` в скриптах `forge:install` / `forge:upgrade` в `package.json` или вызови `forge install` вручную.

На Linux/macOS вместо `npm run env:load` задай переменные в текущей сессии терминала:
...
Или подгрузи весь `.env` разом (из корня проекта):

```bash
set -a && source .env && set +a
```

После этого в том же терминале: `forge login`, затем `npm run deploy`.
...
npm run forge:install      # первая установка на сайт (site в package.json)
...
Скопируй `.env.example` → `.env` и заполни `FORGE_EMAIL` и `FORGE_API_TOKEN`.
...
- [x] Обновление UI после ответа API
  - [ ] Оптимистичные обновления (не реализованы; обновление после ответа API)
- [x] Dockerfile + docker-compose.yml (на машине автора не проверялся — ошибка 0x800f0922 при установке WSL2)
